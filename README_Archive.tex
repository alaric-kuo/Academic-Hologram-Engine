...
\title{README}
\author{Alaric Kuo}
\date{2026-04-25 20:24:31 Taipei Standard Time}
\begin{document}
\maketitle
\begin{abstract}
主題錨點：Academic Value Hologram, Knowledge Ontology Evolution

核心論述：The Academic Value Hologram (AVH) proposes a multi-dimensional, topological framework to measure knowledge evolution potential, moving beyond traditional citation metrics.

全文檢索簽名：Full-text observation, six-dimensional state tensor, and background field interference calculation.
...
\subsubsection{實務層（Practical Layer）}
...
\$\$\textbackslash\{\}text\{Score\}= 0.40 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{probe\}\}+ 0.40 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{signature\}\}+ 0.10 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{anchor\}\}+ 0.10 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{title-topic\}\}\$\$

其中：

- \$S\_\{\textbackslash\{\}text\{probe\}\}\$：該視角 probe 與文獻文本的餘弦相似度  
- \$S\_\{\textbackslash\{\}text\{signature\}\}\$：全文檢索簽名與文獻文本的餘弦相似度  
- \$S\_\{\textbackslash\{\}text\{anchor\}\}\$：實作／應用證據詞與文獻文本的重疊比例  
- \$S\_\{\textbackslash\{\}text\{title-topic\}\}\$：文獻標題與主題錨點／核心論述的對位分數  

若文獻仍缺乏摘要、只能靠標題降維比對，系統會再施加一個緩衝係數：

\$\$
\textbackslash\{\}text\{Score\}\_\{\textbackslash\{\}text\{title-only\}\} = 0.88 \textbackslash\{\}cdot \textbackslash\{\}text\{Score\}
\$\$
...
\$\$\textbackslash\{\}begin\{aligned\}\textbackslash\{\}text\{Global Score\} =\{\}\& 0.52 \textbackslash\{\}cdot S\_\{\textbackslash\{\}max\}+ 0.18 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{avg\}\}+ 0.10 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{sig-max\}\} + 0.05 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{probe-max\}\}+ 0.07 \textbackslash\{\}cdot S\_\{\textbackslash\{\}text\{title-topic-max\}\} + 0.05 \textbackslash\{\}cdot \textbackslash\{\}min(n\_s, 4)+ 0.03 \textbackslash\{\}cdot \textbackslash\{\}min(n\_h, 4)\textbackslash\{\}end\{aligned\}\$\$
...
- \$S\_\{\textbackslash\{\}text\{title-topic-max\}\}\$：標題主題對位分數的最高值  
- \$n\_s\$：命中該文獻的不同來源視角數（source\textbackslash\{\}\_count）  
- \$n\_h\$：該文獻在正式背景池中的總命中次數（hit\textbackslash\{\}\_count）  
...
\$\$\textbackslash\{\}text\{Global Proximity\}=100-\textbackslash\{\}left(\textbackslash\{\}left(\textbackslash\{\}frac\{\textbackslash\{\}text\{Angle\}\}\{1.8\}\textbackslash\{\}right)\textbackslash\{\}times 0.4+\textbackslash\{\}text\{Global Mean Diff\}\textbackslash\{\}times 0.6\textbackslash\{\}right)\$\$
