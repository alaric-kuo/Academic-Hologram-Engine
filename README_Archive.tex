...
\title{README}
\author{Alaric Kuo}
\date{2026-04-24 20:50:11 Taipei Standard Time}
\begin{document}
\maketitle
\begin{abstract}
核心論述：AVH treats academic knowledge as a dynamic energy field, not static data points.

全文檢索簽名：Academic Value Hologram (AVH) protocol for quantifying knowledge ontology evolution, utilizing a six-dimensional state tensor derived from eight fundamental axioms (Space, Time, Entropy, Energy, Mass, Potential) to measure disruptive academic value beyond traditional citation metrics.
...
這套系統不依賴科技巨頭的黑箱神經網路，而是以本地端模型、可追溯的檢索流程，以及顯性數學規則來完成全文觀測。最新版本的引擎已進入 **V55 摘要主權版**：它不再把無摘要文獻當成正式背景母體，也不再讓標題件混入向量干涉計算，而是要求正式背景場必須由「可驗證摘要」所構成。這代表 AVH 的外部場域不是撞字列表，而是可追溯、可量化、可驗證的摘要支撐場。

\subsubsection{第一階段：全文直讀、本體量化與八重探針展開}
...
在這一階段，系統不再用 26 個關鍵字去二次拼裝核心句，而是要求模型直接根據全文生成 8 句不同角度的探針。這樣做的目的，是避免中文本體被壓縮成過度簡化的英文詞場，也避免讓單一句子在早期就主導整個外部場域。

若系統發現 **應用實相（\$D\_6\$）** 被誤判為小於等於 0，但全文中其實已存在明確的實作證據，例如本地端引擎、Crossref 打撈、餘弦相似度計算、JSON 管線、觀測日誌、HTML 實體與 LaTeX 原始碼等，則會啟動一次專門的 **應用實相校正器**，重新判斷第六維，避免將已具體落地的方法論錯誤壓回「純概念停留」的守成狀態。

\subsubsection{第二階段：多視角外發、摘要補完與正式背景篩選}

引擎不押寶單一檢索詞，而是將 8 句有效探針同時向 Crossref 發射。每個視角最多打撈 8 篇文獻，因此原始外部場的最大規模為：

\$\$
N\_\{\textbackslash\{\}max\} = 8 \textbackslash\{\}times 8 = 64
\$\$
...
在文獻重排時，系統不只看單一 probe 與文獻的相似度，還同時考慮全文檢索簽名與實作／應用 anchor 詞的重疊程度。對於每一篇候選文獻，其綜合命中分數定義為：

\$\$
\textbackslash\{\}text\{Score\} = 0.45\textbackslash\{\},S\_\{\textbackslash\{\}text\{probe\}\} + 0.45\textbackslash\{\},S\_\{\textbackslash\{\}text\{signature\}\} + 0.10\textbackslash\{\},S\_\{\textbackslash\{\}text\{anchor\}\}
\$\$

其中：

- \$S\_\{\textbackslash\{\}text\{probe\}\}\$：該視角 probe 與文獻的餘弦相似度  
- \$S\_\{\textbackslash\{\}text\{signature\}\}\$：全文檢索簽名與文獻的餘弦相似度  
- \$S\_\{\textbackslash\{\}text\{anchor\}\}\$：實作／應用證據詞與文獻文本的重疊比例  

若文獻仍缺乏摘要、只能靠標題降維比對，系統會再施加一個懲罰係數：

\$\$
\textbackslash\{\}text\{Score\}\_\{\textbackslash\{\}text\{title-only\}\} = 0.88 \textbackslash\{\}times \textbackslash\{\}text\{Score\}
\$\$

但在 V55 中，這個分數僅用於殘影記錄，不再作為正式背景池的入場券。只有同時滿足下列兩個條件的文獻，才被允許進入正式背景場：

\$\$
\textbackslash\{\}text\{Score\} \textbackslash\{\}geq 0.08
\textbackslash\{\}quad \textbackslash\{\}text\{and\} \textbackslash\{\}quad
\textbackslash\{\}text\{Abstract Available\} = 1
\$\$

這代表 AVH 對背景能勢的要求已經從「撞到就算」提升為「必須可驗證」。

\subsubsection{第三階段：正式背景池聚合、去重與 Top 8 收斂}
...
\$\$
\textbackslash\{\}text\{GlobalScore\}
=
0.55\textbackslash\{\},S\_\{\textbackslash\{\}max\}
+
0.20\textbackslash\{\},S\_\{\textbackslash\{\}text\{avg\}\}
+
0.10\textbackslash\{\},S\_\{\textbackslash\{\}text\{sig-max\}\}
+
0.05\textbackslash\{\},S\_\{\textbackslash\{\}text\{probe-max\}\}
+
0.05\textbackslash\{\},\textbackslash\{\}min(n\_s,4)
+
0.03\textbackslash\{\},\textbackslash\{\}min(n\_h,4)
+
0.02
\$\$
...
- \$S\_\{\textbackslash\{\}max\}\$：該文獻在所有視角中的最高綜合分數  
- \$S\_\{\textbackslash\{\}text\{avg\}\}\$：該文獻在所有命中視角中的平均綜合分數  
- \$S\_\{\textbackslash\{\}text\{sig-max\}\}\$：全文檢索簽名的最高相似度  
- \$S\_\{\textbackslash\{\}text\{probe-max\}\}\$：單一 probe 的最高相似度  
- \$n\_s\$：命中該文獻的不同視角數  
- \$n\_h\$：總命中次數  

最後的常數 \$0.02\$ 表示「摘要驗證加權」；由於正式背景池中的所有節點都已通過摘要可驗證門檻，因此這一項在 V55 中是固定保留的結構獎勵。
...
\subsubsection{第四階段：背景張量量化與物理干涉計算}
...
整體語意相近度（Global Semantic Proximity）則融合了角度懲罰與平均能勢差，定義為：

\$\$
\textbackslash\{\}text\{Global Proximity\}
=
100
-
\textbackslash\{\}left(
\textbackslash\{\}left(\textbackslash\{\}frac\{\textbackslash\{\}text\{Angle\}\}\{1.8\}\textbackslash\{\}right)\textbackslash\{\}times 0.4
+
\textbackslash\{\}text\{Global\textbackslash\{\}\_Mean\textbackslash\{\}\_Diff\}\textbackslash\{\}times 0.6
\textbackslash\{\}right)
\$\$
...
\subsubsection{第五階段：結果顯化、出版輸出與主權定錨}
...
在本地版本中，若工作目錄存在 Git 倉庫，系統還可自動執行 `git add`、`git commit` 與 `git push`，把觀測歷史直接定錨到版本控制之中。這表示 AVH 已不只是文本分析器，而是一套從全文觀測、外場干涉、背景量化到輸出出版的完整主權算力管線；而在 V55 中，這條管線更進一步確立了「摘要主權」：**沒有摘要，就不能進背景場；有殘影，只能當殘影。**
