...
\title{README}
\author{Alaric Kuo}
\date{2026-04-25 09:22:12 Taipei Standard Time}
\begin{document}
\maketitle
\begin{abstract}
主題錨點：Academic Value Hologram, Knowledge Ontology Evolution

核心論述：The Academic Value Hologram (AVH) proposes a multi-dimensional, non-linear protocol to quantify knowledge ontology evolution, moving beyond traditional, static academic metrics.

全文檢索簽名：Full-text analysis using a multi-stage, multi-source protocol for knowledge ontology evolution and academic value quantification.
...
\$\$\textbackslash\{\}text\{GlobalScore\}=0.55\textbackslash\{\},S\_\{\textbackslash\{\}max\}+0.20\textbackslash\{\},S\_\{\textbackslash\{\}text\{avg\}\}+0.10\textbackslash\{\},S\_\{\textbackslash\{\}text\{sig-max\}\}+0.05\textbackslash\{\},S\_\{\textbackslash\{\}text\{probe-max\}\}+0.05\textbackslash\{\},\textbackslash\{\}min(n\_s,4)+0.03\textbackslash\{\},\textbackslash\{\}min(n\_h,4)+0.02\$\$
...
最後的常數 \$0.02\$ 表示「摘要驗證加權」；由於正式背景池中的所有節點都已通過摘要可驗證門檻，因此這一項在系統中是固定保留的結構獎勵。
