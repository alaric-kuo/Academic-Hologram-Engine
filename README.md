...
抓回文獻後，系統不採信資料庫原有的演算法排序。我們將所有文獻的摘要（若無摘要則觸發標題降維比對），重新與我們的 80 維核心矩陣計算 Cosine 相似度。所有 Cosine $\geq 0.1$ 的文獻被允許進入全域候選池。系統接著對全域池進行絕對的相似度降冪排序，並執行嚴格的 DOI 與標題去重，最終坍縮出全域最強的 Top 8 背景節點。
...
$$
\text{Global Proximity} = 100 - \left( \left(\frac{\text{Angle}}{1.8}\right) \times 0.4 + \text{Global\_Mean\_Diff} \times 0.6 \right)
$$
